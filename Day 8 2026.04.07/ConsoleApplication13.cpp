...

{
	...
	if (pid == 0) {
		...
		for (int i = 1; i < 4; i++) {
			...
			MPI_Send(&sequence2[i * 8], 8, MPI_INT, i, 50, MPI_COMM_WORLD);
...

		{
			...
			MPI_Recv(&local_ctn, i, MPI_INT, i, 50, MPI_COMM_WORLD, &sta);
...

	{
		...
		MPI_Recv(&recv_sequence2, 8, MPI_INT, 0, 50, MPI_COMM_WORLD, &sta);
		...
		MPI_Send(&ctn, 1, MPI_INT, 0, 50, MPI_COMM_WORLD);
